...
Minimal package list to run `npm run build`, `npm run install-extension`, and `npm run lint` inside a fresh `fedora-toolbox` container. Does **not** cover the nested GNOME Shell session — run that on the host instead.
...
`git` and `dbus-tools` (for `dbus-run-session`) are already in the default toolbox image.
...
After `npm run install-extension`, log out of the host GNOME session and back in to load the new extension. The toolbox shares `$HOME`, so the host GNOME Shell picks up the install automatically.
